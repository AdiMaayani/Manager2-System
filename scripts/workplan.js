...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (workplanShell) {
    workplanShell.addEventListener('wheel', (e) => {
  ...
  const projectFilterDropdown = document.getElementById('workplan-project-filter');

  function setRole(role) {
    currentRole = role;
    if (roleLabel) {
      const roleName = role === 'employee' ? 'עובד' : role === 'ceo' ? 'רביב' : 'מנהל';
  ...
  function extractTaskData() {
    ...
    document.querySelectorAll('.task[data-task-id]').forEach(task => {
      ...
      const assignee = row ? (row.querySelector('.row-title')?.textContent || '') : '';
      ...
      const assignedEmployeeIds = assignedRaw ? assignedRaw.split(',').map(s => s.trim()).filter(Boolean) : [];
  ...
  function renderWeeklyView() {
    ...
    const employees = [...new Set(tasks.map(t => t.assignee).filter(Boolean))];
    ...
    employees.forEach(employee => {
      ...
      days.forEach((day, dayIndex) => {
        ...
        dayTasks.forEach(task => {
          ...
          if (task.projectId) taskEl.setAttribute('data-project-id', task.projectId);
          if (task.serviceCallId) taskEl.setAttribute('data-service-call-id', task.serviceCallId);
          if (task.assignedEmployeeIds && task.assignedEmployeeIds.length) taskEl.setAttribute('data-assigned-ids', task.assignedEmployeeIds.join(','));
  ...
  function renderMonthlyView() {
    ...
    while (currentDate <= lastDay || week.length < 7) {
      ...
      if (currentDate > lastDay && week.length > 0 && week[0].getMonth() !== currentMonth) {
        break;
      }
    }
    if (week.length > 0) {
    ...
    weeks.forEach(weekDates => {
      ...
      weekDates.forEach(date => {
        ...
        const dayTasksList = tasks.filter((t, idx) => idx % 7 === dayOfWeek).slice(0, 3);
  ...
  function renderYearlyView() {
    ...
    monthNames.forEach((monthName, monthIndex) => {
      ...
      const taskCount = Math.floor(tasks.length / 12) + (monthIndex < tasks.length % 12 ? 1 : 0);
      ...
      monthCard.addEventListener('click', () => {
        ...
        const monthlyBtn = document.querySelector('.tab-range[data-range="monthly"]');
  ...
  const rangeButtons = document.querySelectorAll('.tab-group-range [data-range]');
  ...
  rangeButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      views.forEach(v => v.classList.toggle('active', v.getAttribute('data-range') === range));
      closeDetails();
      updateWorkplanTitle();
  ...
  // Scope switching (WHO)
  const scopeButtons = document.querySelectorAll('.tab-group-scope [data-scope]');
  ...
  const employeeRows = document.querySelectorAll('.workplan-mode-employees .workplan-row[data-row-type="employee"]');
  const CURRENT_USER_ROW_KEY = 'david';

  function applyScope(scope) {
    scopeButtons.forEach(b => b.classList.remove('active'));
    const activeBtn = document.querySelector(`.tab-group-scope [data-scope="${scope}"]`);
    ...
    if (scope === 'project') {
      ...
      employeeRows.forEach(row => {
        if (scope === 'personal') {
          row.style.display = row.getAttribute('data-row-key') === CURRENT_USER_ROW_KEY ? 'grid' : 'none';
        } else {
  ...
  function getCanonicalTaskElement(taskId) {
    ...
    const inEmployees = document.querySelector('.workplan-mode-employees .task[data-task-id="' + taskId + '"]');
    const inProjects = document.querySelector('.workplan-mode-projects .task[data-task-id="' + taskId + '"]');
  ...
  function renderEditAssignedChips() {
    ...
    editModeAssigned.forEach((w, idx) => {
      ...
      chip.innerHTML = '<span class="wp-assigned-chip-label">' + (w.name || '').replace(/</g, '&lt;') + '</span><span class="wp-assigned-chip-remove" aria-hidden="true">×</span>';
      chip.style.cssText = 'display:inline-flex;align-items:center;gap:0.25rem;padding:0.2rem 0.5rem;margin:0.15rem;border-radius:999px;background:var(--color-neutral-200, #e5e7eb);font-size:0.875rem;border:1px solid transparent;cursor:pointer;';
      chip.addEventListener('click', (e) => {
        const t = e.target.nodeType === 3 ? e.target.parentElement : e.target;
        if (!t || !t.classList || !t.classList.contains('wp-assigned-chip-remove')) return;
        e.preventDefault();
  ...
  function enterEditMode() {
    ...
    if (editAssignee) editAssignee.value = assigneeEl ? assigneeEl.textContent : '';
    ...
    if (currentTaskElement) {
      ...
      if (editRole) editRole.value = currentTaskElement.getAttribute('data-role') || '';
    }
    populateAssignedSelect(editModeAssigned.map(x => x.id));
    renderEditAssignedChips();
  ...
  function saveTask() {
    ...
    if (assignedIds.length) target.setAttribute('data-assigned-ids', assignedIds.join(','));
    ...
    if (assignedWorkersEl) {
      assignedWorkersEl.textContent = editModeAssigned.length ? editModeAssigned.map(w => w.name).join(', ') : '—';
    }
    if (roleEl) roleEl.textContent = roleVal || '—';
    if (canonical !== currentTaskElement) {
      if (assignedIds.length) currentTaskElement.setAttribute('data-assigned-ids', assignedIds.join(','));
  ...
  if (editAssignedAdd && editAssignedSelect) {
    editAssignedAdd.addEventListener('click', () => {
      ...
      const name = (opt && opt.dataset.name) ? opt.dataset.name : (opt ? opt.textContent : '');
  ...
  function buildQuickReportPrefill(taskEl) {
    ...
    const rowTitle = row ? (row.querySelector('.row-title, .workplan-weekly-name')?.textContent || '').trim() : '';
    ...
    } else if (projectAttr === 'abc' || name.includes('ABC') || name.includes('משרדי')) {
      projectId = 'PRJ-004';
    ...
    let start = '';
    let end = '';
    const timeMatch = meta.time && meta.time.match(/(\d{1,2}):(\d{2})[-–](\d{1,2}):(\d{2})/);
    if (timeMatch) {
      start = String(parseInt(timeMatch[1], 10)).padStart(2, '0') + ':' + timeMatch[2];
      end = String(parseInt(timeMatch[3], 10)).padStart(2, '0') + ':' + timeMatch[4];
    } else {
    ...
    const assignee = row && !projectAttr ? (row.querySelector('.row-title, .workplan-weekly-name')?.textContent || '').trim() : '';
    const serviceCallId = taskEl.getAttribute('data-service-call-id') || null;
    const role = taskEl.getAttribute('data-role') || null;

    const payload = { date, projectId, customerName, site, start, end, assigneeName: assignee || '', serviceCallId: serviceCallId || undefined, role: role || undefined };
  ...
  if (actionQuickReport) {
    actionQuickReport.addEventListener('click', () => {
      ...
      if (!prefill) return;
      try {
        console.groupCollapsed('🔍 [DATA AUDIT] Quick-report payload passed to reports');
        ...
        console.log('Stored in sessionStorage, navigating to reports.html?quick=1');
        console.groupEnd();
  ...
  function updatePanelActions(taskEl) {
    ...
    if (actionQuickReport) actionQuickReport.style.display = isTaskReportRelevant(taskEl) ? '' : 'none';
    ...
    if (permsEl) {
      if (currentRole === 'employee') {
        ...
        permsEl.textContent = 'הרשאות מנהל: ניתן לערוך הכל ולנעול משימות (עובדים לא יכולים לעקוף נעילה).';
      }
    }

    if (actionHint) {
      if (!editable && currentRole === 'employee') {
        actionHint.textContent = 'כדי לשנות משימה שאינה אישית/נעולה — פנה למנהל.';
      } else if (editable) {
        actionHint.textContent = 'גרירה/שינוי זמן משימה מתאפשרים רק לפי הרשאות.';
      } else {
  ...
  function getStatusBadgeClass(status) {
    ...
    if (statusLower.includes('מתוכנן') || statusLower.includes('תכנון')) return 'badge-neutral';
    if (statusLower.includes('בביצוע') || statusLower.includes('ביצוע')) return 'badge-primary';
    if (statusLower.includes('הושלם') || statusLower.includes('סיום')) return 'badge-success';
    if (statusLower.includes('תקוע') || statusLower.includes('בעיה')) return 'badge-danger';
  ...
  function openTaskPanel(taskEl) {
    ...
    if (row) {
      const rowTitle = row.querySelector('.row-title, .workplan-weekly-name')?.textContent || '';
    ...
    const taskNameEl = taskEl.querySelector('.task-name');
    const name = taskNameEl ? taskNameEl.textContent : (taskEl.textContent.trim().split('\n')[0] || '-');
    const taskMetaEl = taskEl.querySelector('.task-meta');
    const metaText = taskMetaEl ? taskMetaEl.textContent : '';
    const meta = parseMeta(metaText);
    ...
    if (statusEl) {
      ...
      const actualStatus = meta.urgency && meta.urgency !== '-' ? meta.urgency : (meta.status && meta.status !== '-' ? meta.status : '-');
      ...
      if (statusBadge) {
        ...
        badge.className = actualStatus && actualStatus !== '-' 
    ...
    if (linkedEl) {
      if (name.includes('וילה רמת אביב')) {
        linkedEl.textContent = 'פרויקט: וילה רמת אביב • לקוח: וילה רמת אביב • שירות: לפי צורך';
      } else if (name.includes('ABC')) {
        linkedEl.textContent = 'פרויקט: משרדי חברת ABC • לקוח: חברת ABC בע״מ • שירות: #2456';
      } else if (name.includes('מלון דן')) {
        linkedEl.textContent = 'פרויקט: מלון דן תל אביב • לקוח: מלון דן • ציוד: ארון תקשורת';
      } else {
        linkedEl.textContent = 'משימה פנימית/משרדית (ללא קישור לפרויקט).';
      }
    }
    const assignedRaw = taskEl.getAttribute('data-assigned-ids') || '';
    const assignedIds = assignedRaw ? assignedRaw.split(',').map(s => s.trim()).filter(Boolean) : [];
    if (assignedWorkersEl) {
      assignedWorkersEl.textContent = assignedIds.length ? assignedIds.map(id => employeeNameById(id)).join(', ') : '—';
    }
  ...
  function handleTaskClick(e) {
    ...
    const taskEl = e.target.closest('.task[data-task-id], .workplan-weekly-task[data-task-id], .workplan-monthly-task[data-task-id]');
  ...
  function updateTaskInteractivity() {
    document.querySelectorAll('.task[data-task-id]').forEach(task => {
      const locked = task.getAttribute('data-locked') === 'true';
      const personal = task.getAttribute('data-personal') === 'true';
      const allowed = currentRole !== 'employee' ? !locked : (personal && !locked);
  ...
  function snapToHour(task) {
    ...
    if (meta) {
      ...
      const durationHours = Math.max(1, Math.round(task.offsetWidth / hourWidth));
  ...
  function updateWorkplanTitle() {
    ...
    const scope = activeScopeBtn ? (activeScopeBtn.getAttribute('data-scope') || 'company') : 'company';
    const range = activeRangeBtn ? (activeRangeBtn.getAttribute('data-range') || 'daily') : 'daily';
