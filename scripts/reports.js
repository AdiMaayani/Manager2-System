(function () {
  ...
  var MOCK_SERVICE_CALLS = (window.CONTACTS_MOCK_DATA && window.CONTACTS_MOCK_DATA.SERVICE_CALLS) || [];
  var CUSTOMERS = (window.CONTACTS_MOCK_DATA && window.CONTACTS_MOCK_DATA.CUSTOMERS) || [];
  ...
  function initRefs() {
    ...
    el.serviceCallWrap = document.getElementById('report-form-service-call-wrap');
    el.projectLinkedWrap = document.getElementById('report-form-project-linked-wrap');
    ...
    el.followupReasonWrap = document.getElementById('report-followup-reason-wrap');
    ...
    el.viewHeaderActions = document.getElementById('report-view-header-actions');
    ...
    el.relatedWorkersSelect = document.getElementById('report-related-workers-select');
    el.relatedWorkersAddBtn = document.getElementById('report-related-workers-add-btn');
    el.relatedWorkersChips = document.getElementById('report-related-workers-chips');
  }
  ...
  function populateEmployees() {
    ...
    console.groupCollapsed('🔍 [DATA AUDIT] Employees populated in reporter dropdown');
  ...
  function getReportType() {
    if (el.reportTypeService && el.reportTypeService.checked) return 'service_call';
    return 'project';
  }
  ...
  function applyReportTypeUI() {
    ...
    if (el.serviceCallWrap) el.serviceCallWrap.style.display = isProject ? 'none' : '';
  ...
  function customerNameById(customerId) {
    ...
    var c = CUSTOMERS.filter(function (x) { return x.id === customerId; })[0];
    return (c && c.name) ? c.name : '';
  }

  function projectNameById(projectId) {
    if (!projectId) return '';
    var p = MOCK_PROJECTS.filter(function (x) { return x.id === projectId; })[0];
  ...
  function onServiceCallChange() {
    ...
    var customerId = (opt && opt.dataset.customerId) ? opt.dataset.customerId : '';
  ...
  function renderFormRelatedWorkersChips() {
    ...
    formRelatedWorkers.forEach(function (w, idx) {
      ...
      chip.innerHTML = '<span class="report-related-worker-chip-label">' + (w.name || '').replace(/</g, '&lt;') + '</span><span class="report-related-worker-chip-remove" aria-hidden="true">×</span>';
      chip.addEventListener('click', function (e) {
        var t = e.target.nodeType === 3 ? e.target.parentElement : e.target;
        if (!t || !t.classList || !t.classList.contains('report-related-worker-chip-remove')) return;
        e.preventDefault();
  ...
  function initRelatedWorkersForm() {
    ...
    if (el.relatedWorkersAddBtn && el.relatedWorkersSelect) {
      el.relatedWorkersAddBtn.addEventListener('click', function () {
        ...
        if (!v) return;
        if (formRelatedWorkers.some(function (w) { return w.id === v; })) return;
        var opt = el.relatedWorkersSelect.options[el.relatedWorkersSelect.selectedIndex];
        var name = (opt && opt.dataset.name) ? opt.dataset.name : opt ? opt.textContent : '';
  ...
  function getFormReportDraft() {
    ...
    if (reportType === 'project') {
      ...
      var scOpt = el.serviceCall && el.serviceCall.options[el.serviceCall.selectedIndex];
      if (scOpt && scOpt.value) {
        serviceCallId = scOpt.value;
        serviceCallTitle = (scOpt.dataset && scOpt.dataset.title) ? scOpt.dataset.title : '';
        var linkedProjectId = (scOpt.dataset && scOpt.dataset.projectId) ? scOpt.dataset.projectId : '';
        if (linkedProjectId) {
    ...
    var systems = [];
    if (el.systems) {
      el.systems.querySelectorAll('.report-system-chip.selected').forEach(function (chip) {
    ...
    var draft = {
      ...
      reporterName: (el.reporter && el.reporter.options[el.reporter.selectedIndex]) ? el.reporter.options[el.reporter.selectedIndex].textContent : '',
      role: (document.getElementById('report-role') && document.getElementById('report-role').value) || '',
      ...
      followupReason: (document.getElementById('report-followup-reason') && document.getElementById('report-followup-reason').value) || ''
    };
    return draft;
  }
  ...
  function renderRecentReports() {
    ...
    MOCK_RECENT_REPORTS.forEach(function (r) {
      ...
      tr.innerHTML =
        '<td>' + formatDateYmd(r.date) + '</td>' +
        '<td>' + (r.reportNumber || '') + '</td>' +
        '<td>' + (r.projectName || '') + '</td>' +
        '<td>' + (r.customerName || '') + '</td>' +
        '<td>' + (r.reporterName || '') + '</td>' +
        '<td><span class="badge ' + badgeClass + ' report-status-badge">' + status + '</span></td>';
      el.recentTbody.appendChild(tr);
    });
  }

  var expandIconSvg = '<svg viewBox="0 0 24 24" fill="currentColor" width="16" height="16" aria-hidden="true"><path d="M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z"/></svg>';
  var collapseIconSvg = '<svg viewBox="0 0 24 24" fill="currentColor" width="16" height="16" aria-hidden="true"><path d="M5 16h3v3h2v-5H5v2zm3-8H5v2h5V5H8v3zm6 11h2v-3h3v-2h-5v5zm2-11V5h-2v5h5V8h-3z"/></svg>';
  var pencilIconSvg = '<svg viewBox="0 0 24 24" fill="currentColor" width="16" height="16" aria-hidden="true"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/></svg>';
  var checkIconSvg = '<svg viewBox="0 0 24 24" fill="currentColor" width="16" height="16" aria-hidden="true"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>';

  function getReport() {
    if (!selectedReportId) return null;
    return MOCK_RECENT_REPORTS.filter(function (r) { return r.id === selectedReportId; })[0] || null;
  }

  var STATUS_OPTIONS = ['טיוטה', 'הוגש', 'הועבר להנה״ח'];

  var EDETAILS_SYSTEMS = ['חשמל חכם', 'בקרה', 'תקשורת', 'מולטימדיה', 'מצלמות ואבטחה', 'מתנים', 'NVR', 'ארונות תקשורת'];

  var removeIconSvg = '<svg viewBox="0 0 24 24" fill="currentColor" width="14" height="14" aria-hidden="true"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg>';
  ...
  function buildDetailsContent(report, editMode) {
    ...
    if (report.serviceCallId || report.serviceCallTitle) {
      ...
      serviceCallCompact = (sid && stit) ? 'קריאת שירות: ' + sid + ' – ' + stit : (sid || stit) ? 'קריאת שירות: ' + (sid || stit) : '';
    }
    var metaItems = [
      ...
      serviceCallCompact ? { l: '', v: serviceCallCompact, valueOnly: true } : { l: 'קריאת שירות', v: '—' },
    ...
    metaItems.forEach(function (x) {
      ...
      if (x.valueOnly) {
        ...
        item.innerHTML = '<div class="report-view-meta-value">' + (x.v || '—') + '</div>';
      } else if (x.status) {
        ...
        if (editMode) {
          ...
          val.innerHTML = '<span class="badge ' + x.badgeClass + ' report-view-status-badge">' + x.statusVal + '</span>';
          item.appendChild(val);
        }
      } else if (editMode && x.editKey === 'site') {
        item.innerHTML = '<span class="report-view-meta-label">אתר / מיקום עבודה</span>';
        ...
        item.innerHTML = '<span class="report-view-meta-label">' + x.l + '</span><div class="report-view-meta-value">' + (x.v || '—') + '</div>';
      }
    ...
    if (editMode) {
      report.systems.forEach(function (s) {
        ...
        c.textContent = s;
        c.dataset.system = s;
        c.innerHTML = '<span class="report-view-chip-label">' + s + '</span><span class="report-view-chip-remove" aria-hidden="true">×</span>';
        chips.appendChild(c);
      });
    ...
    secWorkers.innerHTML = '<h3 class="report-view-section-title">עובדים קשורים</h3>';
    var workersChips = document.createElement('div');
    workersChips.className = 'report-view-chips report-view-related-workers-chips';
    if (!report.relatedWorkers) report.relatedWorkers = [];
    if (editMode) {
      report.relatedWorkers.forEach(function (w, idx) {
        var c = document.createElement('button');
        c.type = 'button';
        c.className = 'report-view-chip report-view-chip-removable report-view-worker-chip';
        c.dataset.idx = String(idx);
        c.dataset.id = w.id || '';
        c.innerHTML = '<span class="report-view-chip-label">' + (w.name || '').replace(/</g, '&lt;') + '</span><span class="report-view-chip-remove" aria-hidden="true">×</span>';
        workersChips.appendChild(c);
      });
      secWorkers.appendChild(workersChips);
      var addWrapW = document.createElement('div');
      addWrapW.className = 'report-view-systems-add report-view-related-workers-add';
    ...
    if (editMode || hasProducts) {
      ...
      secD.innerHTML = '<h3 class="report-view-section-title">פירוט מערכות מוצרים וחומרים</h3>';
      ...
      if (editMode) {
        report.products.forEach(function (row, idx) {
          ...
          var catSel = '<select class="select report-view-edit-cat" data-idx="' + idx + '"><option value="">בחר קטגוריה</option>' + catOpts + '</select>';
          var prodOpts = MOCK_PRODUCTS.map(function (p) {
            var sel = (row.productId === p.id || row.productName === p.name) ? ' selected' : '';
            return '<option value="' + p.id + '" data-name="' + (p.name || '').replace(/"/g, '&quot;') + '"' + sel + '>' + (p.name || '') + '</option>';
          }).join('');
          var prodSel = '<select class="select report-view-edit-prod" data-idx="' + idx + '"><option value="">בחר מוצר/רכיב</option>' + prodOpts + '</select>';
          var qtyVal = row.qty != null ? String(row.qty) : '';
          var locVal = (row.location || '').replace(/"/g, '&quot;');
          var notesVal = (row.notes || '').replace(/"/g, '&quot;');
          tr.innerHTML =
            '<td>' + catSel + '</td><td>' + prodSel + '</td>' +
            '<td><input type="number" class="input report-view-edit-qty" data-idx="' + idx + '" value="' + qtyVal + '" min="0"></td>' +
            '<td><input type="text" class="input report-view-edit-loc" data-idx="' + idx + '" value="' + locVal + '"></td>' +
            '<td><input type="text" class="input report-view-edit-notes" data-idx="' + idx + '" value="' + notesVal + '"></td>' +
            '<td class="report-view-product-remove-cell"><button type="button" class="btn btn-icon-only btn-sm report-view-remove-row" data-idx="' + idx + '" aria-label="הסר שורה">' + removeIconSvg + '</button></td>';
        ...
        report.products.forEach(function (row) {
          var tr = document.createElement('tr');
          tr.innerHTML =
            '<td>' + (row.category || '—') + '</td><td>' + (row.productName || '—') + '</td>' +
            '<td>' + (row.qty != null ? row.qty : '') + '</td><td>' + (row.location || '') + '</td><td>' + (row.notes || '') + '</td>';
          tbody.appendChild(tr);
        });
        tableWrap.appendChild(tbl);
        secD.appendChild(tableWrap);
    ...
    if (report.followup) {
      ...
      secFollow.innerHTML = '<h3 class="report-view-section-title">דורש ביקור חוזר</h3><div class="report-view-meta-value">' + (report.followupReason || '—') + '</div>';
    ...
    secF.innerHTML =
      ...
      '<input type="checkbox" class="report-view-accounting-checkbox" ' + (report.transferredToAccounting ? 'checked' : '') + '>' +
  ...
  function renderDetailsHeader(report) {
    ...
    el.viewReportId.textContent = report.reportNumber ? '#' + report.reportNumber : '';
    ...
    if (isEditMode) {
      ...
      saveBtn.innerHTML = '<span class="report-view-action-icon">' + checkIconSvg + '</span>שמירה';
      saveBtn.addEventListener('click', function () {
        ...
        var notesEl = el.viewBody.querySelector('.report-view-edit-notes-global');
        var statusEl = el.viewBody.querySelector('.report-view-edit-status');
        var accCheck = el.viewBody.querySelector('.report-view-accounting-checkbox');
        ...
        if (r.products) {
          rows.forEach(function (tr, i) {
            ...
            if (prodSel) {
              ...
              r.products[i].productName = (opt && opt.dataset.name) ? opt.dataset.name : '';
            }
            if (qtyEl) {
      ...
      expandBtnEdit.className = 'btn btn-outline btn-sm report-view-expand-btn' + (isDetailsExpanded ? ' report-view-expand-btn-hidden' : '');
      expandBtnEdit.title = 'הגדל מסך';
      expandBtnEdit.innerHTML = '<span class="report-view-action-icon">' + expandIconSvg + '</span>הגדל מסך';
      ...
      collapseBtnEdit.className = 'btn btn-outline btn-sm report-view-collapse-btn' + (isDetailsExpanded ? '' : ' report-view-collapse-btn-hidden');
      collapseBtnEdit.title = 'הקטן מסך';
      collapseBtnEdit.innerHTML = '<span class="report-view-action-icon">' + collapseIconSvg + '</span>הקטן מסך';
      ...
      editBtn.innerHTML = '<span class="report-view-action-icon">' + pencilIconSvg + '</span>עריכה';
      ...
      expandBtn.className = 'btn btn-outline btn-sm report-view-expand-btn' + (isDetailsExpanded ? ' report-view-expand-btn-hidden' : '');
      expandBtn.title = 'הגדל מסך';
      expandBtn.innerHTML = '<span class="report-view-action-icon">' + expandIconSvg + '</span>הגדל מסך';
      ...
      collapseBtn.className = 'btn btn-outline btn-sm report-view-collapse-btn' + (isDetailsExpanded ? '' : ' report-view-collapse-btn-hidden');
      collapseBtn.title = 'הקטן מסך';
      collapseBtn.innerHTML = '<span class="report-view-action-icon">' + collapseIconSvg + '</span>הקטן מסך';
  ...
  function wireDetailsBody(report) {
    var accountingCheck = el.viewBody.querySelector('.report-view-accounting-checkbox');
    ...
    if (accountingCheck) {
      accountingCheck.addEventListener('change', function () {
        ...
        report.status = accountingCheck.checked ? 'הועבר להנה״ח' : (report.status === 'הועבר להנה״ח' ? 'הוגש' : report.status);
        if (statusBadge) {
          statusBadge.textContent = report.status;
          statusBadge.className = 'badge ' + statusBadgeClass(report.status) + ' report-view-status-badge';
        }
        if (statusSelect) {
    ...
    if (addRowBtn && report) {
      addRowBtn.addEventListener('click', function () {
        if (!report.products) report.products = [];
        report.products.push({ category: '', productId: '', productName: '', qty: 0, location: '', notes: '' });
        renderDetailsModal();
      });
    }

    /* Remove row (items table) */
    el.viewBody.querySelectorAll('.report-view-remove-row').forEach(function (btn) {
      btn.addEventListener('click', function () {
        var idx = parseInt(btn.dataset.idx, 10);
        if (isNaN(idx) || !report.products || idx < 0 || idx >= report.products.length) return;
    ...
    var addSystemSel = el.viewBody.querySelector('.report-view-add-system-select');
    ...
    el.viewBody.querySelectorAll('.report-view-chip-removable[data-system]').forEach(function (chip) {
      chip.addEventListener('click', function (e) {
        var t = e.target.nodeType === 3 ? e.target.parentElement : e.target;
        if (!t || !t.classList || !t.classList.contains('report-view-chip-remove')) return;
        e.preventDefault();
        var s = chip.dataset.system;
    ...
    var addWorkerSel = el.viewBody.querySelector('.report-view-add-worker-select');
    if (addWorkerBtn && addWorkerSel && report) {
      if (!report.relatedWorkers) report.relatedWorkers = [];
      addWorkerBtn.addEventListener('click', function () {
        var v = addWorkerSel.value;
        if (!v) return;
        if (report.relatedWorkers.some(function (w) { return w.id === v; })) return;
    ...
    el.viewBody.querySelectorAll('.report-view-worker-chip').forEach(function (chip) {
      chip.addEventListener('click', function (e) {
        var t = e.target.nodeType === 3 ? e.target.parentElement : e.target;
        if (!t || !t.classList || !t.classList.contains('report-view-chip-remove')) return;
        e.preventDefault();
        var idx = parseInt(chip.dataset.idx, 10);
        if (isNaN(idx) || !report.relatedWorkers || idx < 0 || idx >= report.relatedWorkers.length) return;
  ...
  function openViewModal(id) {
    var report = MOCK_RECENT_REPORTS.filter(function (r) { return r.id === id; })[0];
    if (!report || !el.viewBody || !el.viewModal) return;
  ...
  function initFormModal() {
    ...
    document.addEventListener('keydown', function (e) {
      if (e.key === 'Escape' && el.formModal && el.formModal.classList.contains('active')) {
        closeFormModal();
      }
    });
  }
  ...
  function prefillFormFromQuickReport(prefill) {
    if (!prefill) return;
    var defaultService = !!(prefill.serviceCallId && String(prefill.serviceCallId).trim());
    ...
    if (!defaultService && el.customer && prefill.customerName) el.customer.value = prefill.customerName;
    ...
    if (el.reporter && prefill.assigneeName) {
      var match = MOCK_EMPLOYEES.filter(function (e) { return e.fullName === prefill.assigneeName; })[0];
      if (match) el.reporter.value = match.id;
    }
    if (el.role && prefill.role) {
      var roleText = (typeof prefill.role === 'string' ? prefill.role : '').trim();
      if (roleText) {
    ...
    if (el.systems) {
      el.systems.querySelectorAll('.report-system-chip').forEach(function (chip) {
  ...
  function tryOpenQuickReportForm() {
    ...
    console.groupCollapsed('🔍 [DATA AUDIT] Quick-report payload received from workplan');
    var raw;
    try {
