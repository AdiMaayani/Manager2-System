...
SELECT COUNT(*) InvalidTypeCategoryRows FROM dbo.WorkItems WHERE NOT(IsArchived=1 OR (WorkType=N'Project' AND TaskCategory IS NULL AND ParentWorkItemId IS NULL AND MilestoneId IS NULL) OR (WorkType=N'Task' AND TaskCategory=N'Regular' AND ParentWorkItemId IS NULL AND MilestoneId IS NULL) OR (WorkType=N'Task' AND TaskCategory=N'Project' AND ParentWorkItemId IS NOT NULL) OR (WorkType=N'ServiceCall' AND TaskCategory=N'ServiceCall' AND ParentWorkItemId IS NULL AND MilestoneId IS NULL));
...
BEGIN
 IF EXISTS(SELECT 1 FROM dbo.WorkItems WHERE NOT(IsArchived=1 OR (WorkType=N'Project' AND TaskCategory IS NULL AND ParentWorkItemId IS NULL AND MilestoneId IS NULL) OR (WorkType=N'Task' AND TaskCategory=N'Regular' AND ParentWorkItemId IS NULL AND MilestoneId IS NULL) OR (WorkType=N'Task' AND TaskCategory=N'Project' AND ParentWorkItemId IS NOT NULL) OR (WorkType=N'ServiceCall' AND TaskCategory=N'ServiceCall' AND ParentWorkItemId IS NULL AND MilestoneId IS NULL))) THROW 51440,'Cannot enable WorkItems checks while invalid rows remain.',1;
